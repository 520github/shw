 ...
 * 
 */
package com.babeeta.appstore.android.crawler.validate.gfan;
...
/**
 * GFan市场HTML详情页面的样式格式校验
 * @author xuehui.miao
 *
 */
public class GFanApplicationDetailStyleValidate extends HtmlStyleValidate<GFanApplicationDetail> {
	private GFanApplicationDetail detail;
	
	public GFanApplicationDetailStyleValidate(InputStream inputStream,
			GFanApplicationDetailVisitor visitor) {
	...
	public boolean validateStyle() {
		...
		if(message.length() > 0) {
			message.insert(0, "GFan市场HTML详情页面的样式格式发生变更,部分内容无法解析,具体如下:\n");
		}
		return validate;
	}
}
